...
<template>
  ...
  >
      ...
      <div class="flex lg:flex-1">
        <a href="#" class="-m-1.5 p-1.5">
          ...
          <span class="flex items-center gap-x-2">
            ...
            <span class="text-white text-base font-semibold">Mermeme Coin</span>
          </span>
        </a>
      </div>
      <div class="flex lg:hidden">
        <button type="button" class="-m-2.5 inline-flex items-center justify-center rounded-md p-2.5 text-gray-300 hover:bg-green-800/30 transition-colors"
    ...
    <Dialog class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel
        class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-[#084f1b] px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-700/10">
        <div class="flex items-center justify-between">
          <a href="#" class="-m-1.5 p-1.5">
            <span class="sr-only">Mermeme Coin</span>
            <img class="h-8 w-auto" :src="MermemeLogo" alt="" />
          </a>
          <button type="button" class="-m-2.5 rounded-md p-2.5 text-gray-300 hover:bg-green-800/30" @click="mobileMenuOpen = false">
            <span class="sr-only">Close menu</span>
            <XMarkIcon class="size-6" aria-hidden="true" />
          </button>
        </div>
        <div class="mt-6 flow-root">
          <div class="-my-6 divide-y divide-green-800/50">
            <div class="space-y-2 py-6">
              <a v-for="item in navigation" :key="item.name" :href="item.href"
                class="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold text-white hover:bg-green-800/30">{{
                item.name }}</a>
            </div>
            <div class="py-6">
              <a href="#"
